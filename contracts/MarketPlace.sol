...
contract MarketPlace is IAuctionHouse, ReentrancyGuard {
    ...
    function buyOrder(uint256 orderId) external payable auctionExists(orderId) nonReentrant {
        ...
        tokenOwnerProfit = tokenOwnerProfit.sub(curatorFee.add(royaltyFee));//the amount which we sold for in that some will go to the platform and some will go to the royality to the first user
    ...
    function endAuction(uint256 auctionId) external override auctionExists(auctionId) nonReentrant {
        ...
        if(checkRoyalties(auctions[auctionId].tokenContract)){
            (address reciver, uint royaltyAmount) = IERC2981(saleOrder[auctionId].tokenContract).royaltyInfo(saleOrder[auctionId].tokenId,tokenOwnerProfit);
